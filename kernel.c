...
char scan_code_to_ascii(unsigned char scan_code) {
    // This is a very simplified mapping, real implementation needs a full scan code table
    if (scan_code == 0x02) return '1'; // Example: Map scan code for '1' key to ASCII
    // Add mappings for other scan codes...
    return 0; // Unmapped scan codes return 0
}
...
void handle_keyboard_input() {
    ...
    if (ascii) {
        print_char(ascii); // Use your existing function to print the character
    }
}
...
void create_dosbox_ui()
{
  ...
  gotoxy(2, 1);
  print_color_string("Welcome to Serinix OS (x86 v1.0)", BRIGHT_GREEN, BLUE);

  gotoxy(2, 3);
  print_color_string("Created by: ", WHITE, BLUE);
  
  gotoxy(21, 3);
  print_color_string("Aryanstha", YELLOW, BLUE);

  gotoxy(2, 4);
  print_color_string("Supported by: ", WHITE, BLUE);
  
  gotoxy(21, 4);
  print_color_string("Aryanstha", YELLOW, BLUE);

  gotoxy(2, 7);
  print_color_string("For more informations view kernel source code", WHITE, BLUE);
  
  gotoxy(2, 10);
  print_color_string("Serinix Micro Kernel", BRIGHT_GREEN, BLUE);

  gotoxy(2, 11);
  print_color_string("Serinix Kernel Source:", BRIGHT_GREEN, BLUE);

  gotoxy(28, 11);
  print_color_string("https://github.com/aryanstha/SerinixOS", YELLOW, BLUE);

  gotoxy(0, 14);
  print_color_string("root@Serinix:>>", WHITE, BLACK);
...
void kernel_entry()
{
  ...
  while (1) {
        ...
        // A real implementation would sleep or yield here, waiting for an interrupt
    }
